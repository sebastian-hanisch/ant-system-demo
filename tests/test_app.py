...
def test_every_preset_button_runs(name):
    ...
    _ok(at)
    p = C.PRESETS[name]
    assert at.session_state["n_slider"] == p["n"] and at.session_state["rho_slider"] == p["rho"]
    assert at.metric
...
def test_generation_slider_runs_at_various_positions():
    ...
    _ok(at)
    gen_slider = next(s for s in at.slider if s.key == "aco_gen")
...
@pytest.mark.parametrize("kw", [dict(n_slider=C.N_MIN), dict(n_slider=C.N_MAX), dict(ants_slider=C.ANTS_MIN), dict(gens_slider=C.GEN_MIN), dict(alpha_slider=C.ALPHA_MIN), dict(beta_slider=C.BETA_MAX), dict(rho_slider=C.RHO_MIN), dict(rho_slider=C.RHO_MAX)])
...
def test_sweep_runs_on_demand():
    at = _run()
    at.selectbox(key="sweep_select").set_value("rho").run()
...
def test_rho_experiment_runs_on_demand(monkeypatch):
    monkeypatch.setattr(C, "RHO_VALUES", (0.05, 0.95))
    monkeypatch.setattr(C, "RHO_EXPERIMENT_SEEDS", (1, 2))
    monkeypatch.setattr(C, "RHO_EXPERIMENT_ANTS", 6)
    monkeypatch.setattr(C, "RHO_EXPERIMENT_GENS", 10)
    at = _run()
    next(b for b in at.button if b.key == "rho_start").click().run()
    _ok(at)
    assert at.session_state["rho_on"] and at.get("plotly_chart")
...
def test_footer_and_grenzen_are_present():
    ...
    assert any("Keine Pheromon-Obergrenze" in m.value for m in at.markdown)
